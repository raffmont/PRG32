...
void prg32_display_init(void) {
    ...
    spi_device_interface_config_t dev = {
        ...
        .mode = 0,
        .spics_io_num = PRG32_PIN_LCD_CS,
        .queue_size = 7
    };
    ESP_ERROR_CHECK(gpio_set_direction(PRG32_PIN_LCD_DC, GPIO_MODE_OUTPUT));
    ESP_ERROR_CHECK(gpio_set_direction(PRG32_PIN_LCD_RST, GPIO_MODE_OUTPUT));
    ESP_ERROR_CHECK(gpio_set_direction(PRG32_PIN_LCD_BL, GPIO_MODE_OUTPUT));
    ESP_ERROR_CHECK(gpio_set_level(PRG32_PIN_LCD_BL, 0));
    ESP_ERROR_CHECK(spi_bus_initialize(SPI2_HOST, &bus, SPI_DMA_CH_AUTO));
    ESP_ERROR_CHECK(spi_bus_add_device(SPI2_HOST, &dev, &g_lcd));
    ESP_ERROR_CHECK(gpio_set_level(PRG32_PIN_LCD_RST, 0));
    esp_rom_delay_us(20000);
    ESP_ERROR_CHECK(gpio_set_level(PRG32_PIN_LCD_RST, 1));
    ...
    lcd_data(&mad, 1);
    lcd_cmd(0x29);
    ESP_ERROR_CHECK(gpio_set_level(PRG32_PIN_LCD_BL, 1));
    dirty_reset();
}
