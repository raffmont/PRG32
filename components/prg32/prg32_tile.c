...
void prg32_tile_put(uint8_t tx, uint8_t ty, uint8_t id) {
    if (tx >= PRG32_TILE_COLS || ty >= PRG32_TILE_ROWS) return;
...
void prg32_tile_present(void) {
    for (int ty=0; ty<PRG32_TILE_ROWS; ++ty) for (int tx=0; tx<PRG32_TILE_COLS; ++tx) {
        if (!g_dirty[ty][tx]) continue;
        g_dirty[ty][tx] = 0;
        tile_t *t = &g_tiles[g_map[ty][tx]];
        for (int row=0; row<8; ++row) for (int col=0; col<8; ++col) {
            uint16_t c = (t->bits[row] & (1 << (7-col))) ? t->fg : t->bg;
            prg32_gfx_pixel(tx*8+col, ty*8+row, c);
        }
    }
    prg32_gfx_present();
}
